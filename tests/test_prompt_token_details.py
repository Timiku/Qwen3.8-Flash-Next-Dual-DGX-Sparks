...
class PromptTokenDetailsTests(unittest.TestCase):
    def test_base_args(self):
        ...
        # Dual kit: the rendered string is consumed by BOTH launch scripts
        # (worker rank 1 + head rank 0), so the marker appears twice.
        self.assertEqual(source.count('$VLLM_ARGS_STR \\'), 2)
