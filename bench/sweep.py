...
Ported from MiaAI-Lab/Qwen3.8-Flash-Next-Single-DGX-Spark (AGPL-3.0-or-later)
bench/sweep.py, which drove the PR #39 reduced-vocabulary A/B there. The
endpoints are identical from this host (sparkDash :5555, vLLM head :8888);
only the serving topology differs (2-node TP2+EP, worker on gigabyte).
...
def run_level(s, prompt, max_tokens):
    while True:
        try:
            ...
            # 409 = another bench job holds the endpoint; 429 = sparkDash's
            # inter-job cooldown. Both clear on their own; poll, don't die.
            if e.code in (409, 429):
                print(f"  sparkDash busy ({e.code}), waiting 10 s", file=sys.stderr); time.sleep(10); continue
            raise
    bid = job["benchId"]
    while True:
        time.sleep(3)
        try:
            ...
            print(f"  poll failed ({e}), retry", file=sys.stderr); continue
